...
const Confirm = () => {
  ...
  useEffect(() => {
    ...
    const checkBookingStatus = async () => {
      try {
        const response = await userInstance.get(`/api/users/check-booking-status`, {
  ...
  const sendPaymentSuccessEmail = async (
    userId, email, amount, currency, receipt, customerName, hotelName, bookingId, checkInDate, checkOutDate, numberOfGuests, paymentDate, paymentTime, customerSupportEmail, customerSupportPhone
  ) => {
    try {
  ...
  return (
    <div className='payment_page_container'>
      <h1>
        <i className='fas fa-chevron-left' style={{ cursor: "pointer" }} onClick={() => navigate(`/properties/${property._id}?name=${property.name}&location=${property.location}`)} /> Confirm and Pay
      </h1>
      {property ? (
            ...
            <div className='payment_card_section'>
              ...
              <div className='price_details'>
                ...
                <div className='total_price'>
                  ...
                  <Button style={{ width: '100%' }} variant='danger' onClick={handlePayment}>
                    {loading ? (
